...
public class WelcomeLoginPage {
    ...
    public void show( Stage primaryStage, User user) {
	    ...
	    continueButton.setOnAction(a -> {
	    	...
	    	// route staff to staff home (admin-equivalent with staff dashboard)
	    	if("admin".equalsIgnoreCase(role)) {
	    		new AdminHomePage(primaryStage, user.getUserName()).show(primaryStage);
	    	}
	    	else if("staff".equalsIgnoreCase(role)) {
	    		new StaffHomePage(primaryStage, user.getUserName()).show(primaryStage);
	    	}
		...
		if ("admin".equals(user.getRole())) {
			...
			inviteButton.setOnAction(a -> {
				// InvitationPage now manages its own Stage internally.
				new InvitationPage().show(databaseHelper);
			});
